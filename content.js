...
async function generateReply(tweetText) {
  ...
  const prompt = `你要在推特上回复下面这条推。目标不是"回得对"，而是"回得让人想点赞/截图"。在心里走完三步，【只输出第三步】。

【第一步 · 写手】用和原推相同的语言回。先读懂原推真正在说什么，然后找一个【钩子】——不是同意也不是反对，而是一个能让人"欸有点意思"的角度。钩子可以是下面任意一种，挑最贴这条推调性的：
